...
function DashboardRail({
  ...
  const [isRecommendationDismissed, setIsRecommendationDismissed] =
    useState(false);
  const usage =
    availableMinutes > 0
  ...
  {hasActiveTasks && (
    <>
        ...
        {!recommendation || isRecommendationDismissed ? (
          ...
          <div className="recommendation-card__task">
            ...
            <div>
              ...
              <p>
                {formatDuration(
                  recommendation.recommended_task.duration_minutes,
                )}{" "}
      ...
      <section
        className="rail-card capacity-status"
      ...
      >
        ...
        <div
          className="capacity-status__track"
          aria-label={`${usage}% of available time planned`}
        >
          <span style={{ width: `${usage}%` }} />
        </div>
        <p>
          {plannedCount} {plannedCount === 1 ? "task" : "tasks"} in today’s plan
        </p>
      </section>
            </>
    )}
      ...
      <div className="rail-pair">
        ...
        >
          ...
          <div className="plant-card__content">
            ...
            <div>
              ...
              <p>
                {progress
                  ? `${progress.completed_tasks} completed tasks`
                  : "Your progress will grow here."}
              </p>
            </div>
          </div>
          <div className="plant-card__progress">
            <span style={{ width: `${progress?.progress_percent ?? 0}%` }} />
          </div>
        </section>

        <section
          className="rail-card reflection-card"
        ...
        >
          ...
          {reflection ? (
            <p>
              {reflection.tasks_completed} completed ·{" "}
              {reflection.tasks_recovered} recovered
            </p>
          ) : (
            <p>Your weekly reflection will appear here.</p>
          )}
          <span className="reflection-card__note">
            A gentle record of your progress.
          </span>
        </section>
      </div>
    </aside>
