...
def add_oauth_accounts_table():
    """Create the oauth_accounts table on existing deployments.

    Fresh databases receive the table automatically from ``Base.metadata.create_all``.
    This migration only runs ALTER/CREATE for databases deployed before OAuth
    support was added. It is idempotent: if the table already exists it returns
...
def add_password_reset_tokens_table():
    """Create the password_reset_tokens table on existing deployments.
...
def make_users_password_hash_nullable():
    """Allow password_hash to be NULL for OAuth-only accounts.

    Existing email/password accounts are unaffected — their password_hash
    column stays populated. Only new OAuth-only users will have NULL there.
    SQLite allows NULL in NOT NULL columns when rows are inserted with NULL
    via Python (the constraint is not enforced at the DB level for ALTER),
    but we drop the NOT NULL constraint explicitly on PostgreSQL to keep the
    schema honest.
    """
    if engine.dialect.name != "postgresql":
        return
